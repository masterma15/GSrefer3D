...
class LazySupervisedSpatialDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        assert enable_dynamic_res == True or enable_dynamic_res_s2 == True, "SpatialDataset do not need to resize image"
        ...
        if "image" in sources[0]:
            ...
            if isinstance(image_file, list):
                if enable_dynamic_res_s2:
                    ...
                    if self.enable_depth:
                        processed_depths, sources[0][0]["value"] = dynamic_process_depths_and_prompt(depth_file, sources[0][0]["value"], self.data_args, self.depth_folder)
                else:
